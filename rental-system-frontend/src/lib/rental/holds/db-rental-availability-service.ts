...
async function listCommittedOrdersForEquipment(equipmentId: string): Promise<OrderRangeRow[]> {
  ...
  const { data, error } = await supabase
    .from(ORDERS_TABLE)
    .select("id,equipment_id,qty,start_date,end_date")
...
export async function getEquipmentAvailabilityForRange(input: {
  ...
  const [committedOrders, holds] = await Promise.all([
  ...
  return {
    ...
    committedQty,
    heldQty,
    availableQty: Math.max(0, config.totalUnits - committedQty - heldQty),
  };
}
